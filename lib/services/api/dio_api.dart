    ...
    try {
      ...
      if (response?.statusCode == HttpStatus.ok) {
        ...
        metaFields.forEach((field) {
          response?.data["docs"][0]["field_map"]["${field["fieldname"]}"] =
              true;
        });
        if (await OfflineStorage.storeApiResponse()) {
    ...
    for (FrappeFile frappeFile in files!) {
      ...
      FormData formData = FormData.fromMap({
        "file": await MultipartFile.fromFile(
          frappeFile.file.path as String,
          filename: fileName,
        ),
        "docname": name,
        "doctype": doctype,
      ...
      if (e is DioException) {
        if (e.response != null &&
          ...
          if (e.error is SocketException) {
            ...
            throw ErrorResponse(
              statusCode: e.error as int,
              statusMessage: e.message as String,
            );
          }
        }
      } else {
    ...
    try {
      ...
      if (e is DioException) {
        ...
        if (error is SocketException) {
          ...
          throw ErrorResponse(statusMessage: e.message as String);
        }
      } else {
        throw ErrorResponse();
      }
